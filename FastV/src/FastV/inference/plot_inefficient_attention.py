...
def visualize_attention(
    multihead_attention, output_path="atten_map_1.png", title="Layer 5"
):
    ...
    averaged_attention = torch.mean(multihead_attention, axis=1)[
        0
    ].float()  # Shape: (n_tokens, n_tokens)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-path", type=str, required=True, default="llava-v1.5-13b"
    )
    parser.add_argument(
        "--image-path", type=str, required=True, help="figs/example.jpg"
    )
    parser.add_argument(
        "--prompt", type=str, required=True, help="discribe the image in detail"
    )
    parser.add_argument(
        "--output-path",
        type=str,
    ...
    model_output, outputs_attention = inference(
