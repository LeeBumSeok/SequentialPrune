...
output_path=./results/docvqa_eval_fastv
mkdir -p $output_path
...
for rank in ${rank_list[@]}; do
    ...
    python ./src/FastV/inference/eval/inference_docvqa.py \
        ...
        --fast-v-agg-layer $k \
        --output-path $output_path/docvqa_7b_FASTV_nocache_mt40_${rank}_${k}.json 
    done
done
