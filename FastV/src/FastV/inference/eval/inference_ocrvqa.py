...
print(f"Total number of examples in the dataset: {N_examples}")
cur_num=0

# target_index = [3637, 3638, 3639, 3640]

# for i in target_index:
#     if i < N_examples:
#         target_data = dataset[i]
#         print(f"Data at index {i}: {target_data}")
#     else:
#         print(f"Index {i} is out of range. The dataset only contains {N_examples} examples.")
...
if __name__=="__main__":
    ...
    target_index = 3639
    target_data = dataset[target_index]

    try:
        image = target_data['image']
        if image.mode != 'RGB':
            image = image.convert('RGB')
        print(f"Processing image at index {target_index} with size: {image.size} and mode: {image.mode}")
        processed_image = image_processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
        print("Image processed successfully.")
    except Exception as e:
        print(f"Error processing image at index {target_index}: {e}")
        print(f"Image details: size - {image.size}, mode - {image.mode}")
    ...
    # %%
    def inference(prompts, images, answers):
        outputs = []
        for prompt, image, gt_answers in tqdm(zip(prompts, images, answers), total=len(prompts)):
            image = image.convert('RGB')
    ...
    ocrvqa_inference_outputs = inference(dataset_ocrvqa_prompts, dataset_ocrvqa_images, dataset_ocrvqa_labels)
    ...
    labels = [item['gt_answers'] for item in ocrvqa_inference_outputs]
    predictions = [item['model_answer'] for item in ocrvqa_inference_outputs]
    score, scores = get_cider(labels, predictions)
    
    for idx, item in enumerate(ocrvqa_inference_outputs):
        item['cider_score'] = scores[idx]

    output_path = pargs.output_path
    with open(output_path, "w") as f:
        json.dump({"avg": str(score), "output": ocrvqa_inference_outputs}, f, indent=4)