...
for i in dataset:
    ...
    valid_questions.append(i["question"])
    valid_choices.append(i["choices"])
    valid_anwser.append(i["correct_choice_idx"])
...
if __name__=="__main__":
    ...
    def compute_acc_with_details(model_output, correct_anwser, questions, images):
        correct = 0
        output_details = []
        for i in range(len(model_output)):
            is_correct = correct_anwser[i] in model_output[i]
            if is_correct:
                correct += 1
            output_details.append({
                "question_id": questions[i]['question_id'],
                "image_path": questions[i]['image_id'],
                "question": questions[i]['question'],
                "model_answer": model_output[i],
                "gt_answers": [correct_anwser[i]],
                "accuracy": int(is_correct)
            })
        return correct/len(model_output), output_details
    # %%


    # %%
    acc, output_details = compute_acc_with_details(oakvqa_val_inference_outputs, valid_anwser_options, dataset, valid_images)

    output_path = pargs.output_path

    with open(output_path, "w") as f:
        json.dump({"avg": str(acc), "output": output_details}, f, indent=4)