...
if __name__ == "__main__":
    ...
    #     interactive=True,
    # )
    attention_Layer = gr.inputs.Dropdown(
        choices=["All layers", "", "Sample 5 layers", "Sample 10 layers"],
        default="Sample 3 layers",
        label="Layer Attention Visualization",
    )
    ...
    prompt_textbox = gr.Textbox(
        label="Prompt", placeholder="Describe the image in detail.", lines=2
    )
    ...
    def inference(image_input, prompt, num_of_layers="All layers"):
        ...
        output = (
            model_output_ori
            if isinstance(model_output_ori, str)
            else model_output_ori[0]
        )
