...
for model in llava-v1.5-13b; do
    ...
    for attention_rank in "${ATTENTION_RANKS[@]}"; do
        for agg_layer in "${AGG_LAYERS[@]}"; do
            for dataset_name in IEdit MMCoQA MovingAttribute MovingDirection MultiModalQA OCR-VQA ObjectExistence ObjectInteraction ObjectShuffle SceneTransition SlideVQA Spot-the-Diff StateChange TQA WebQA nuscenes; do
                BATCH_SIZE=1
