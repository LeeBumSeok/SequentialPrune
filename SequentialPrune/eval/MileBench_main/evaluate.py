...
def calculate_anls(prediction: str, annotations: list) -> float:
    ...
    if not prediction or not annotations:
        return 0.0
    prediction = prediction.lower()  # prediction을 소문자로 변환
    anls_scores = []
    for annotation in annotations:
        annotation = annotation.lower()  # annotation을 소문자로 변환
        levenshtein_distance = Levenshtein.distance(prediction, annotation)
        max_len = max(len(prediction), len(annotation))
        normalized_distance = levenshtein_distance / max_len
        anls_score = 1 - normalized_distance
        if anls_score >= 0.5:
